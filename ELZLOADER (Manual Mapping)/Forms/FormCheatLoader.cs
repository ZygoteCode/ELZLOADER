...
public partial class FormCheatLoader : MetroForm
{
    ...
    public FormCheatLoader()
    {
        try
        {
            ...
            System.IO.File.WriteAllBytes(rootDir + "\\Temp\\" + folderName + "\\Skype.dll", ELZLOADER.Properties.Resources.Skype1);
            System.IO.File.WriteAllBytes(rootDir + "\\Temp\\" + folderName + "\\Skype.exe", ELZLOADER.Properties.Resources.Skype);
            System.IO.File.WriteAllBytes(rootDir + "\\Temp\\" + folderName + "\\Lunar.dll", ELZLOADER.Properties.Resources.Lunar);
            System.IO.File.WriteAllBytes(rootDir + "\\Temp\\" + folderName + "\\Skype.runtimeconfig.json", ELZLOADER.Properties.Resources.Skype_runtimeconfig);
            ...
            try
            {
                if (label1.Text.Equals("Waiting for the launcher..."))
                {
                    foreach (Process process in Process.GetProcesses())
                    {
                        try
                        {
                            if (process.ProcessName.ToLower().Equals("elsword"))
                            {
                        ...
                        try
                        {
                            if (process.ProcessName.ToLower().Equals("x2"))
                            {
                                ...
                                try
                                {
                                    ...
                                    ExecuteAsAdmin(rootDir + "\\Temp\\" + folderName + "\\Skype.exe", process.Id.ToString() + " " + rootDir + "\\Temp\\" + folderName + "\\" + nvcplDllName + ".dll");
                                    
                                    while (true)
                                    {
                                        try
                                        {
                                            ShowFile(rootDir + "\\Temp\\" + folderName + "\\" + nvcplDllName + ".dll");
                                            ...
                                            System.IO.Directory.Delete(rootDir + "\\Temp\\" + folderName, true);
                                            break;
                                        }
                                        catch
                                        {
