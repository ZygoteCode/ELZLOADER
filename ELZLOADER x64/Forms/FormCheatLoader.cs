...
public partial class FormCheatLoader : MetroForm
{
    ...
    private string newSpeedHackDllPath;
    ...
    public FormCheatLoader()
    {
        try
        {
            ...
            {
                newSpeedHackDllPath = rootDir + "\\Temp\\" + folderName + random.GetRandomString(characters, random.GetRandomInt32(6, 18)) + ".dll";
                System.IO.File.WriteAllBytes(newSpeedHackDllPath, ELZLOADER.Properties.Resources.NewSpeedHack);
                HideFile(newSpeedHackDllPath);
            }
                    ...
                    foreach (Process process in Process.GetProcesses())
                    {
                        try
                        {
                            if (process.ProcessName.ToLower().Equals("x2"))
                            {
                                ...
                                {
                                    SpeedHackManager.InjectDLL(processId, newSpeedHackDllPath);
                                    MessageBox.Show("Enjoy!", "Done", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    Process.GetCurrentProcess().Kill();
                                    return;
                                }
