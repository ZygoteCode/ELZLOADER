...
public partial class FormLuaInjector : MetroForm
{
    ...
    public FormLuaInjector()
    {
        try
        {
            ...
            {
                CrashReporting.WriteCrashReport("Detection 29, Illegal violation");
            ...
            {
                CrashReporting.WriteCrashReport("Detection 30, Illegal violation");
                Process.GetCurrentProcess().Kill();
                return;
            }
            ...
            {
                CrashReporting.WriteCrashReport("Detection 31, Illegal violation");
                Process.GetCurrentProcess().Kill();
                return;
            }
            ...
            {
                CrashReporting.WriteCrashReport("Detection 32, Illegal violation");
                Process.GetCurrentProcess().Kill();
                return;
            }
            ...
            {
                CrashReporting.WriteCrashReport("Detection 33, Illegal violation");
                Process.GetCurrentProcess().Kill();
                return;
            }
        }
        ...
        {
            CrashReporting.WriteCrashReport("Detection 35, Illegal violation");
            Process.GetCurrentProcess().Kill();
            return;
        }
        ...
        try
        {
            ...
            if (System.IO.File.Exists(Utils.DataFolder + "\\auto-restart-dungeon.txt"))
            {
                System.IO.File.Delete(Utils.DataFolder + "\\auto-restart-dungeon.txt");
            }
            ...
            PipeHandler.PipeConnect();
            PipeHandler.PipeAuth(Utils.smethod_0(credentials, Utils.smethod_9()), credentials, splitted[0] + "\\\\");

            if (System.IO.Directory.Exists("autorun"))
            {
                foreach (string file in System.IO.Directory.GetFiles("autorun"))
                {
                    PipeHandler.ExecuteLua(System.IO.File.ReadAllText(file));
                }
            }
            ...
            PipeHandler.ExecuteLua(Encoding.UTF8.GetString(ELZLOADER.Properties.Resources.InitialScript));
        ...
        {
            CrashReporting.WriteCrashReport("Detection 34, Illegal violation", ex);
            Process.GetCurrentProcess().Kill();
        }
    }
    ...
    {
        return GetCaptionOfActiveWindow().StartsWith("Elsword - ");
    }
    ...
    {
        PipeHandler.ExecuteLua($"GodModeEnabled = {siticoneCheckBox3.Checked.ToString().ToLower()}");
    }

    private void gunaButton5_Click(object sender, EventArgs e)
    {
        PipeHandler.ExecuteLua("g_pDungeonGame:GetMyUnit():SetNowHP(0)");
    }

    private void siticoneCheckBox5_CheckedChanged(object sender, EventArgs e)
    {
        PipeHandler.ExecuteLua($"AllElixirsHackEnabled = {siticoneCheckBox5.Checked}");
    }
    ...
    {
        PipeHandler.ExecuteLua($"AttackSpeedHackValue = {(GetAttackSpeedHackValue()).ToString()}\r\nAttackSpeedHackEnabled = {siticoneCheckBox7.Checked.ToString().ToLower()}");
    }
    ...
    {
        PipeHandler.ExecuteLua(fastColoredTextBox1.Text);
    }
}